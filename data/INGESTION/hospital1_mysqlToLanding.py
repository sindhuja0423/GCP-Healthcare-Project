...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://10.8.128.3:3306/hospital-1db?useSSL=false&allowPublicKeyRetrieval=true",
...
def get_latest_watermark(table_name):
    query = f"""
        ...
        FROM `{BQ_AUDIT_TABLE}`
        WHERE tablename = '{table_name}' and data_source = "hospital-1db"
    """
...
for row in config_df.collect():
    if row["is_active"] == '1' and row["datasource"] == "hospital-1db": 
